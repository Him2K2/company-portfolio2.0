...
const PageWrapper = memo(({ pageNumber, children, id, sectionId, setActiveSection, isExportingPDF }) => {
  ...
  return (
    <div ref={ref} id={id} className="relative flex-1 w-full min-h-screen bg-white flex flex-col lg:max-w-[80vw] print:min-h-[277mm] print:block print:py-5">
      <div className="flex-grow">
        {children}
      </div>

      {pageNumber > 0 && (
        <div className=" w-screen h-8 bg-blue-600 relative flex justify-center lg:max-w-[80vw] mt-auto">
          <button className="px-10 py-1 text-white bg-stone-900 absolute -top-3 left-1/3 md:absolute md:-top-3 md:left-10">
            Page No: {pageNumber}
          </button>
        </div>
      )}
